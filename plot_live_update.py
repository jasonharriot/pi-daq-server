...
if __name__ == '__main__':
    ...
    graph_T1 = ax1.plot(df['timestamp'], df['T1'], color='red', label='T1')[0]    #Plot T1. Grab element [0] of the return value. Don't need the other values.
    graph_T2 = ax1.plot(df['timestamp'], df['T2'], color='orange', label='T2')[0] #Plot T2
    ...
    anim = matplotlib.animation.FuncAnimation(fig, update, interval=10000, frames = None, fargs = (db, channel_names, df, graph_T1, graph_T2, graph_V1), cache_frame_data = False)
